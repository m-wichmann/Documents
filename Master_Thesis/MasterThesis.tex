...
\section*{Abstract}
After almost ten years since the beginning of AUTOSAR, the architecture and development process has been established in the automotive industry. This master thesis looks at the possibilities to use virtualized AUTOSAR components. This is done by a case study, in which both the virtualization, including the communication, and AUTOSAR as a concept are evaluated. To cover the demand for developers, the Ostfalia University of Applied Sciences included AUTOSAR in its curriculum. Therefore this thesis covers the fundamentals of AUTOSAR and describes the performed case study.
}
...
Daraufhin kann das erläuterte Fallbeispiel im Kapitel \ref{sec:Umsetzung_Fallbeispiel} umgesetzt werden. Wobei als Schwerpunkt vor allem auf die Kommunikation sowie die Virtualisierung eingegangen wird. Die Ergebnisse des Fallbeispiels werden im letzten Kapitel analysiert und es wird untersucht, ob die vorgestellten Techniken eine Zukunft in realen Projekten haben.
...
In diesem Projekt wurde AUTOSAR sowie die Virtualisierung in eingebetteten Systemen evaluiert. Hierzu wurde ein Versuchsaufbau einer Scheinwerfersteuerung verwendet, die über ein virtualisiertes System gesteuert wird. Dieses besteht aus einer AUTOSAR-Komponente und einer in Python erstellten Anwendung. Um die Kommunikation zwischen beiden Systemen zu gewährleisten, wurde die VCAN-API entwickelt, mit der ein einfacher Zugriff auf den VCAN möglich ist. Der Aufbau des Steuergerätes konnte durch Unterstützung der Volkswagen AG in relativ kurzer Zeit durchgeführt werden. Hierdurch konnte das Netzwerkmanagement ebenfalls untersucht, jedoch aus Zeitgründen nicht im endgültigen Projekt implementiert werden.

Zusammenfassend lässt sich sagen, dass die Virtualisierung ein interessantes und ausgereiftes Konzept darstellt, dass durchaus auch in der Welt der eingebetteten Systeme Anwendung finden kann. Zwar wird die Systemkomplexität durch den Einsatz von Virtualisierung erhöht, aber gleichzeitig durch die starke Abstraktion und strikte Trennung ein alternatives Entwicklungskonzept ermöglicht. Hierin lassen sich Teilsysteme bausteinartig einsetzen, während das Gesamtsystem in einem Top-Down-Verfahren entworfen wird. Eine konkrete Kosten-Nutzen-Rechnung muss allerdings in jedem Projekt einzeln erfolgen, da eine allgemeine Aussage nur eingeschränkt möglich ist.

Trotz der großen Komplexität moderner Steuerungsgeräte muss zu jeder Zeit die funktionale Sicherheit, nach ISO 26262, garantiert sein. Der Einsatz dieser Norm ist gesetzlich notwendig, und Bedarf, aufgrund des Umfangs, besonderer Aufmerksamkeit. Aus Sicherheits- und Kostengründen ist es auf lange Sicht notwendig, die ISO 26262 und AUTOSAR zu verwenden. Daher ist es sinnvoll diese Techniken zu verbinden.

In dem hier betrachteten Fallbeispiel hat sich die Virtualisierung als praktisches Konzept gezeigt. Dabei ist zu beachten, dass die wahren Vorteile dieser Technik erst durch den Einsatz eines Type 1-Hypervisors entstehen. Dieses ist jedoch mit viel Aufwand verbunden, wobei XtratuM als freie und offene Lösung einen guten Einstieg bietet. Auch in der Lehre könnte dies in Zukunft eingesetzt werden.

Die in diesem Projekt eingesetzte AUTOSAR-Distribution WinCore, sowie der darin integrierte VCAN, sind sehr gut in der Lehre, sowie in einer ersten Evaluationsphase zu nutzen. Bei längerer Benutzung zeigt sich die Instabilität als kritische Eigenschaft, sodass ein Produktiveinsatz in dieser Weise nicht möglich ist. Beim VCAN handelt es sich um eine einfache und offene Möglichkeit, in die Kommunikation des Systems einzugreifen, um eine Restbussimulation sowie manuelle Tests schnell zu ermöglichen.

An dieser Stelle zeigt sich Potential für zukünftige Projekte. Sollte der VCAN auch weiter eingesetzt werden, bietet es sich an, die Integration der Kommunikationsmatrix, sowie der Systembeschreibung zu realisieren. Hierdurch wäre eine vereinfachte Nutzung, sowie eine schnelle Einrichtung einer einfachen Restbussimulation umsetzbar. Da die in dieser Arbeit erstellte VCAN-API offen gehalten und gut dokumentiert ist, sind entsprechende Anpassungen nach kurzer Einarbeitungszeit implementierbar.

Während Virtualisierung viel Potential zeigt, sehe ich AUTOSAR eher kritisch. Durch den Einfluss vieler Hersteller, ist der Standard sehr umfangreich und komplex geworden. Außerdem befasst sich der Standard mit einigen Implementationsdetails, wie der Entscheidung, eine Funktionalität als Makro oder Funktion zu implementieren. Meiner Meinung nach sollte AUTOSAR einfacher und allgemeiner gehalten sein um die konkrete Implementierung den Produktherstellern zu überlassen. Ebenfalls ist das Fehlen einer Referenzimplementierung ein kritischer Punkt. Erst durch das Implementieren einer Spezifikation, ist es möglich, eventuelle Designfehler und -lücken frühzeitig zu erkennen. Optimal wäre eine offene Implementierung, die als Best Practice für die Industrie gilt.
...
\vspace{2.5cm} \par
Wolfenbüttel, den 18.3.2013% TODO
