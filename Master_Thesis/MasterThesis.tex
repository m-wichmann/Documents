...
% TODO: Notiz Markus (?): Komisches Argument
...
Die Oberfläche der in diesem Projekt entwickelten Anwendungen besteht aus zwei Fenstern. Die Oberfläche ist mittels wxPython realisiert, da dieses Plattformübergreifend ohne Anpassungen unterstützt wird. Das in Abbildung \ref{fig:vcan_app_status} zu sehende Status-Fenster, stellt die Informationen der verwendeten CAN-Botschaften\footnote{\texttt{Licht\_vorne\_01}, \texttt{SMLS\_01} und \texttt{Klemmen\_Status\_01}} in einer übersichtlichen Darstellung dar.
...
% - High Precision Timer könnte bis 100 ns Sekunden runter

% - WxWidget oder pygame für präzise timer -> jitter von 1 ms möglich (unter Linux)
\begin{figure}
    \centering
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{boxplot_as}
        \caption{Timing AUTOSAR}
        \label{fig:boxplot_as}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{boxplot_vcan}
        \caption{Timing VCAN-API}
        \label{fig:boxplot_vcan}
    \end{subfigure}
    \caption{Timinganalyse des VCAN als Boxplot}
    \label{fig:timinganalyse}
\end{figure}

% Größe der stichprobe: 1000 Botschaften
% AUTOSAR zum Teil sehr viel schlechter (Senden in Bursts)
\begin{table}[ht]
    \centering
    \begin{tabular}[h]{r  r r}
            & Stichprobe AUTOSAR & Stichprobe VCAN\\
        \toprule
        Durchschnitt        & 122 ms & 100 ms\\
        Median              & 120 ms & 100 ms\\
        Maximal Wert           & 140 ms & 100 ms\\
        Minimal Wert           & 113 ms & 97 ms\\
        25\% Quantil        & 119 ms & 100 ms\\
        75\% Quantil        & 129 ms & 100 ms\\
        Standard Abweichung & 4 ms & 0 ms\\
        \bottomrule
    \end{tabular}
