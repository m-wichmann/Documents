...
Zusammenfassend lässt sich sagen, dass die Virtualisierung ein interessantes und ausgereiftes Konzept darstellt, dass durchaus auch in der Welt der eingebetteten Systeme Anwendung finden kann. Zwar wird die Systemkomplexität durch den Einsatz von Virtualisierung erhöht, jedoch wird gleichzeitig durch die starke Abstraktion und strikte Trennung ein alternatives Entwicklungskonzept ermöglicht. Hierin lassen sich Teilsysteme bausteinartig einsetzen, während das Gesamtsystem in einem Top-Down-Verfahren entworfen wird. Eine konkrete Kosten-Nutzen-Rechnung muss allerdings in jedem Projekt einzeln erfolgen, da eine allgemeine Aussage nur eingeschränkt möglich ist.

Trotz der großen Komplexität moderner Steuerungsgeräte muss zu jeder Zeit die funktionale Sicherheit garantiert sein. Die funktionale Sicherheit nach ISO 26262 enthält einige interessante Punkte. Der Einsatz dieser Norm ist außerdem gesetzlich notwendig, und Bedarf aufgrund des Umfangs ein besonderes Augenmerk. Da auch ein Umstieg auf AUTOSAR auf lange Sicht notwendig ist, bietet es sich an, die ISO 26262 und AUTOSAR von Beginn an zu verbinden.
...
Während Virtualisierung viel Potential zeigt, sehe ich AUTOSAR eher kritisch. Durch den Einfluss vieler Hersteller, ist der Standard sehr umfangreich und komplex geworden. Außerdem befasst sich der Standard mit einigen Implementationsdetails, wie der Entscheidung, eine Funktionalität als Makro oder Funktion zu implementieren. Meiner Meinung nach sollte AUTOSAR einfacher und allgemeiner gehalten sein und die konkrete Implementierung den Produktherstellern zu überlassen. Ebenfalls ist das Fehlen einer Referenzimplementierung ein kritischer Punkt. Erst durch das Implementieren einer Spezifikation, ist es möglich, eventuelle Designfehler und Designlücken frühzeitig zu erkennen. Optimal wäre eine offene Implementierung, die als Best Practice für die Industrie gelten kann.
